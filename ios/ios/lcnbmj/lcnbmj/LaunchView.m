    ...
    if( self != nil )
    {
        ...
        for (UIView* subView in weakSelf.view.subviews) {
            if ([subView isKindOfClass:[UIImageView class]]) {
                ...
                //imageView.image = [UIImage imageNamed:@"Launch.imageset"];
            }
        }
...
//为了点击弹框后重新访问 window.loadGameUrl
-(void)showAlert:(NSString*)msg
{
    BlockUIAlertView *alertView = [[BlockUIAlertView alloc] initWithTitle:NSLocalizedString(@"", @"") message:NSLocalizedString(msg, msg) cancelButtonTitle:NSLocalizedString(@"重试", @"重试") clickButton:^(NSInteger indexButton){
        if (indexButton == 0)
        {
            [MyJSBridge callRunJs:@"window.loadConchUrl(window.loadGameUrl)"];
        }
    ...
    } otherButtonTitles:NSLocalizedString(@"前往设置", @"前往设置")];
    [alertView show];
}
