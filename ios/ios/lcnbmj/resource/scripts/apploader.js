...
var XMLHttpRequest = (function (_super) {
    ...
    XMLHttpRequest.prototype.open = function (type, url, async, isFullPath) {
        console.log('xhr.' + type + ' url=' + url);
        if (!url)
            return;
        type = type.toUpperCase();
        async = true;
        if(!isFullPath) url = location.resolve(url);
...
var Document = (function (_super) {
            ...
            window.downloadfile(d.src, false, function (data) {
                ...
                setTimeout(function()
                {
                    if(window.LogManager)
                        window.LogManager.uploadFlowLog("加载["+d.src+"]失败", "加载流程");
                }, 1000);
    ...
    Document.prototype._downloadOk = function (d) {
        ...
        for (var i = this._evalNum, len = this.scriptTextList.length; i < len; i++) {
            ...
            {
                if(window.LogManager)
                    window.LogManager.uploadFlowLog("加载["+t.src+"][耗时:="+dt+"]", "加载流程");
            }, 7000);
            var e = new Event("load");
